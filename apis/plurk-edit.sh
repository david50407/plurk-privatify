...
function usage {
  echo "usage: $0 [options] id
    ...
    -l list,   --limit-to=list     Change plurk limitation to <list> (in '[<idx>, <idx>, <idx>]' form)
  "
}

OPTS=`getopt -o ht:s:l: --long help,app:,app-secret:,token:,secret:,limit-to: -n "$0" -- "$@"`
...
while true; do
  case "$1" in
    ...
    -t | --token ) TOKEN="$2" ;;
    -s | --secret ) SECRET="$2" ;;
    -l | --limit-to ) LIMIT_TO="$($TOOLS/urlencode.sh "$2")" ;;
    -- ) shift; break ;;
    * ) break ;;
  esac
  shift 2
...
TARGET_ID="$1"
[ "$TARGET_ID" == "" ] && usage && exit 1

METHOD=POST
ENDPOINT="https://www.plurk.com/APP/Timeline/plurkEdit"

QUERIES=(
  "plurk_id=$TARGET_ID"
)
[ "$LIMIT_TO" != "false" ] && QUERIES+=("limited_to=$LIMIT_TO")
